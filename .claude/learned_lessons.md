...
### Issue 12: pro-components silently falls back to Chinese UI text when the browser locale doesn't exactly match a supported key
- **Description**: A brand-new ProTable page (Accounts > Customers) rendered its search form and toolbar in Chinese ("请输入" placeholders, "重置"/"查询"/"展开" buttons) even though `menu.ts`/`pages.ts` translations for the page were correct and present in `en-US`/`de-DE`/`es-ES`.
- **Root Cause**: `config/config.ts`'s `locale` plugin had `baseNavigator: true`, which overrides the configured `default: 'en-US'` with the browser's raw `navigator.language` on first load. This app only ships exactly three locale keys (`en-US`, `de-DE`, `es-ES` — see `src/.umi/plugin-locale/localeExports.ts`'s generated `localeInfo` map). Any browser reporting something that doesn't match one of those three keys exactly (a bare `en`, `en-GB`, `fr`, etc.) misses the `localeInfo[locale]` lookup, so `src/.umi/plugin-locale/locale.tsx` passes antd's `ConfigProvider` an empty `{}` locale object. `@ant-design/pro-components` reads its own built-in strings from that same `ConfigProvider` context and, finding no recognized locale, silently falls back to its hardcoded default — which is Chinese. This is invisible on `useIntl`-driven app text (still correct, since umi's own `IntlProvider` always falls back to the *configured* default, not the browser string) but visible on any pro-components built-in chrome (search/reset/expand buttons, `ProFormText` placeholders, etc.), so it can hide for a long time on pages that don't use those.
- **Solution/Skill**: Set `baseNavigator: false` in `config/config.ts`'s `locale` block. `getLocale()` (`src/.umi/plugin-locale/localeExports.ts`) already prefers a saved `localStorage.umi_locale` choice over the browser string, so disabling `baseNavigator` only removes the *unmatched-browser-string* fallback path — the language switcher (`LangDropdown` in `app.tsx`) still works and still persists to `localStorage`. Verify with a browser whose OS/browser language is NOT one of the three supported locales: load any ProTable page and confirm search/reset/expand render in English, not Chinese. This is a global config change (not page-scoped) — re-check other ProTable pages (`table-list`, `llm-performance`) after changing it, since they may have been silently affected too and never noticed.

### Issue 13: Don't assert an exact status code for AJV type-confusion/coercion test payloads — assert the security invariant instead
- **Description**: A test for `POST /api/accounts/login` sent several "type confusion" payloads (object email, array email, numeric password, `null` password, `null` email) in one loop, asserting every single one returns `400`. It failed twice, on two DIFFERENT payloads each time (`null` email first, then — after moving `null`/number to their own case and asserting the OBJECT/ARRAY shapes are "guaranteed 400" — one of THOSE failed too, still expecting 400, actually getting 401).
- **Root Cause**: Fastify's default AJV instance runs with `coerceTypes: true`, which DOES coerce some scalars into a schema-valid string (e.g. `null` → `""`) before the handler runs, so a coerced `null` email can legitimately reach the DB lookup and come back `401 invalid_credentials` — the safe, correct outcome, just not `400`. The mistake compounded: after fixing the `null` case, a SECOND wrong guess assumed `object`/`array` shapes are always AJV-unresolvable and therefore reliably 400 — that guess was ALSO wrong for at least one of those shapes, without ever being confirmed by actually running the suite. **The real lesson isn't "here is AJV's exact coercion table" — two attempts at stating that table from memory were both wrong.** The lesson is: don't hand-derive AJV/Fastify's exact validation-vs-coercion behavior for a payload shape and bake that guess into a specific `expect(res.statusCode).toBe(N)` assertion unless you've actually run the test and seen the number. Any assumption made without running it is a guess, and this codebase's own AJV version/config is the only real authority.
- **Solution/Skill**: For "does this weird-shaped payload get rejected" tests, assert the INVARIANT that actually matters for the route's security, not a specific status code path you haven't verified: e.g. for an auth route, `expect(res.statusCode).not.toBe(200)` (never a bypass) + `expect(res.statusCode).toBeLessThan(500)` (never a crash) + "if 401, the body is exactly the generic `{status:'invalid_credentials'}`" (never leaks which check actually failed). This is correct regardless of whether Fastify/AJV happens to reject the payload at the schema layer or lets it fall through to real business logic — both are acceptable outcomes; only a 200 or a 500 would be a real bug. Reserve an exact `toBe(400)` assertion for payloads you've actually watched fail schema validation in a real test run (missing required fields, this route's own declared `maxLength`/`minLength` on an otherwise-valid string) — not for object/array/null "this should surely be rejected" guesses.

### Issue 14: Old test fixture's `ON CONFLICT (parent_id, key)` breaks once a partial unique index is added for NULL-parent rows
- **Description**: After fixing the categories re-seed bug with a partial unique index (`categories_root_key_uidx ON categories (key) WHERE parent_id IS NULL`, since SQL's `UNIQUE (parent_id, key)` never treats `NULL = NULL` as a conflict), 13 pre-existing tests in the `apparel item editor` describe block started failing with `duplicate key value violates unique constraint "categories_root_key_uidx"`.
- **Root Cause**: A test fixture (`createTestItem`'s `INSERT INTO categories (level, key, label) VALUES (0, 'men', 'Men') ON CONFLICT (parent_id, key) DO NOTHING`) predates the partial index and targets the old full-table `UNIQUE (parent_id, key)` constraint. Postgres requires an `ON CONFLICT` target to exactly match the constraint/index that would actually fire; a NULL-parent row's real conflict is on the *partial* index, which `ON CONFLICT (parent_id, key)` cannot suppress — so once the real taxonomy seed in `schema.sql` had already inserted `'Men'`/`'Women'`/etc. at level 0, this fixture's insert (a different-case `'men'`) exposed the mismatch (a duplicate for whichever test ran it twice, or for any level-0 key that DOES collide).
- **Solution/Skill**: When adding a partial unique index to replace/supplement a full-table one, grep the whole test suite for every raw `INSERT ... ON CONFLICT` against that table (not just the schema/migration files) and update each target to match: `ON CONFLICT (key) WHERE parent_id IS NULL DO NOTHING` for the NULL-parent case, leaving non-null-parent inserts on the original `ON CONFLICT (parent_id, key)` (that constraint is still real and still fires normally). Always run the FULL test suite after adding a partial index — a single describe block's isolated pass is not enough evidence, since these fixtures are typically buried many hundreds of lines away from the schema change itself.

### Issue 15: A fixed (non-per-request) Redis cache key makes rapid repeated test runs flaky
- **Description**: New tests for `GET /public/catalog/brands` and a pre-existing `api health check` test (checkout-quote via the unfiltered `GET /public/catalog/feed?limit=20`) intermittently failed with stale data/`422` errors ONLY when re-running the same test file multiple times within a couple of minutes — a single clean run always passed.
- **Root Cause**: These routes cache their response under a key that's either entirely fixed (`catalog:public:categories`/`brands`/`colors` — no per-request variation at all) or fixed for a given call shape (`GET .../feed?limit=20` with no filters always hashes to the same key). Their TTLs (120–300s) comfortably outlive a fast iterative `vitest run` → fix → `vitest run` loop during development, so a later run's assertions were validated against an EARLIER run's cached response body, not the current DB state.
- **Solution/Skill**: For tests against a fixed-cache-key route, either (a) vary an actual request parameter that's part of the cache key so each test run gets a fresh key (e.g. the campaigns endpoint's `?slot=` — always use a fresh `randomUUID()`-suffixed slot), or (b) expose the app's `ICacheManager` at module scope in the test file (`let cache: ICacheManager | undefined;` assigned in the same `beforeAll` that calls `createCacheManager`) and call `await cache?.delete('the:exact:key')` right before the assertion. For a full clean run (not just one test), flush the whole cache first: `docker compose exec redis redis-cli flushall`, or the new `npm run cache:flush` / `npm run test:clean` scripts in `server/package.json` (added specifically because of this issue) which flush Redis via `server/db/flushCache.ts` before invoking vitest.

### Issue 16: `ILogger.error()`'s argument order is `(msg, fields)`, not pino's native `(fields, msg)`
- **Description**: `log.error({ err }, 'failed to flush redis cache')` — the idiom used everywhere pino is called directly in this codebase (e.g. `req.log.error({ err }, '...')` inside Fastify route handlers) — fails `tsc` with `Argument of type '{ err: unknown; }' is not assignable to parameter of type 'string'` when called on the app's own `ILogger` abstraction instead of a raw pino instance.
- **Root Cause**: `src/core/ILogger.ts`'s interface is `error(msg: string, fields?: LogFields): void` — message first, structured fields second — the OPPOSITE order from pino's native `logger.error(mergingObject, msg)`. Fastify's `req.log` inside a route handler IS a real pino instance (native order is correct there), but the app's own `logger`/`logger.child(...)` (used in standalone scripts like `db/*.ts`, outside any request) is the `ILogger` wrapper, not raw pino.
- **Solution/Skill**: Outside a Fastify request handler (any `db/*.ts` CLI script, `core/*.ts` module-level code), always call the app logger as `log.error('human message', { err })` / `log.info('message', { fields })` — message string first. Only `req.log`/`app.log` inside an actual route handler take the native pino `(mergingObject, msg)` order. When in doubt, check which type is actually in scope (`ILogger` import vs a raw Fastify request) rather than assuming the pino convention everywhere.

### Issue 17: `SELECT ... LIMIT 1` with no `ORDER BY` is not stable once a table has non-representative leftover rows
- **Description**: A new test picked "a real level-0 category" via `SELECT id FROM categories WHERE level = 0 LIMIT 1` (no `ORDER BY`) to build a category-tree filter test. It failed intermittently with "no descendant category found" even though the real seeded taxonomy's level-0 rows all have real subtrees.
- **Root Cause**: `categories` is a fixed-reference table that is deliberately never truncated between test files/runs (see Issue-adjacent doc comments in `db/seed.ts`), so ad-hoc, tree-less rows inserted by OTHER tests' fixtures (e.g. Issue 14's `'men'`/lowercase row, which has no `category_tree` entries at all) persist across the whole suite's lifetime in a shared dev/test database. Without `ORDER BY`, Postgres's sequential scan can return ANY matching row first depending on physical tuple order — confirmed via `EXPLAIN` returning the highest-id (most recently inserted, tree-less) row ahead of the real low-id seeded ones.
- **Solution/Skill**: Never rely on unordered `LIMIT 1` against a table that other tests are known to insert extra, non-representative rows into. Either add an explicit `ORDER BY id ASC` (real seed data reliably has the lowest ids, since it's inserted once at `schema.sql` load time before any test runs) or, more robustly, constrain the query itself to only match rows with the actual required shape — e.g. `JOIN category_tree ... WHERE depth = 1` so a row that structurally can't satisfy the test (no children) is never a candidate regardless of ordering.
